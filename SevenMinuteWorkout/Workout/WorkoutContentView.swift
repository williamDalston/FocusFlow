...
struct WorkoutContentView: View {
        ...
        ZStack {
            ...
            ScrollViewReader { proxy in
                ScrollView(showsIndicators: false) {
                    ...
                    .padding(.top, horizontalSizeClass == .regular ? DesignSystem.Spacing.xl : 0)
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.lg) {
            ...
            .overlay(RoundedRectangle(cornerRadius: DesignSystem.CornerRadius.large, style: .continuous).stroke(Theme.strokeOuter, lineWidth: DesignSystem.Border.subtle))
            .shadow(color: Theme.enhancedShadow.opacity(DesignSystem.Opacity.light), radius: DesignSystem.Shadow.medium.radius, y: DesignSystem.Shadow.medium.y)
            .frame(maxWidth: .infinity)
        }
    }
    ...
    private func configureEngineFromPreferences() {
        ...
        // Configure from preset if selected
        if let preset = prefs.selectedPreset {
            engine.configure(with: preset)
        }
        // Configure from custom workout if selected
        else if let customWorkoutId = prefs.selectedCustomWorkoutId,
                let customWorkout = preferencesStore.getCustomWorkout(id: customWorkoutId) {
            engine.configure(with: customWorkout)
        }
        // Configure from preferences
        else {
            engine.configure(
                exerciseDuration: prefs.exerciseDuration,
                restDuration: prefs.restDuration,
                prepDuration: prefs.prepDuration,
                skipPrepTime: prefs.skipPrepTime,
                exerciseIds: nil // Use all exercises by default
            )
        }
    }
}
