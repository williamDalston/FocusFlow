...
final class WatchSessionManager: NSObject, ObservableObject {
    ...
    func sendNewSessionToWatch(_ session: WorkoutSession) {
        guard let session = session,
        ...
        let message = [
            "action": "add_session",
            "session": (try? JSONEncoder().encode(session)) as Any
        ] as [String : Any]
        ...
        session.sendMessage(message, replyHandler: { response in
            DispatchQueue.main.async {
                ...
                for session in sessions {
                    if !self.workoutStore?.sessions.contains(where: { $0.id == session.id }) ?? false {
            ...
            case "add_session":
                if let data = message["session"] as? Data,
                   let workoutSession = try? JSONDecoder().decode(WorkoutSession.self, from: data) {
                    ...
                    if !self.workoutStore?.sessions.contains(where: { $0.id == workoutSession.id }) ?? false {
                        self.workoutStore?.addSession(
